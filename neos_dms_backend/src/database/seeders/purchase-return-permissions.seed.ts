...
/**
 * Phase 7a: adds the `purchase.return.*` codes and re-ensures every base
 * role's mappings. Accountant and manager get full return control via the
 * `purchase.return.*` glob (create/read/update/post/void); warehouse manager
 * stays GRN-only; salesman has no purchase access. Idempotent.
 */
export const purchaseReturnPermissionsSeed: Seed = {
  version: 22,
  name: 'purchase-return-permissions-backfill',
  async run(manager) {
