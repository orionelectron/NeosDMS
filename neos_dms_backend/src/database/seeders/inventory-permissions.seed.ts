...
/**
 * Phase 5f: adds the `inventory.location.*` and `inventory.balance.read`
 * codes, then re-ensures every base role's mappings so warehouse_manager
 * (via `inventory.*`) gets full access while manager gets read-only stock
 * visibility. `inventory.transaction.{create,read,adjust}` already exist.
 * Idempotent — mirrors the earlier backfills as a new version.
 */
export const inventoryPermissionsSeed: Seed = {
  version: 17,
  name: 'inventory-permissions-backfill',
  async run(manager) {
