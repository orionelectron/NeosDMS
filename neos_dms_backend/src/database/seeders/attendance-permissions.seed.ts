...
/**
 * Phase C2 follow-up: adds the `hr.attendance.*` permission codes, then
 * re-ensures every base role's mappings so salesman gains self-service
 * attendance (check-in/out, own reports) and manager keeps team-wide access
 * via `hr.*`. Idempotent — mirrors v12/v13 as a new version so already-seeded
 * DBs pick up the diff.
 */
export const attendancePermissionsSeed: Seed = {
  version: 15,
  name: 'attendance-permissions-backfill',
  async run(manager) {
    ...
    // Correct v14: `hr.attendance.adjust` is manager-only. v14 had granted
    // salesman the whole `hr.attendance.*` glob (including `adjust`, which
    // also guards the org-wide `/attendance/all` list); revoke it now so the
    // permission set converges with BASE_ROLES.
    await manager.query(
      `DELETE FROM role_permission_mappings
       WHERE role_id IN (SELECT id FROM roles WHERE code = 'salesman')
         AND permission_id IN (SELECT id FROM permissions WHERE code = 'hr.attendance.adjust')`,
    );
  },
};
