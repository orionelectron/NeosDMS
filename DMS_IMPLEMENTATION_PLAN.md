...
- [x] POST transaction: AR/VAT journal (DR AR 1103 + party, CR Sales 4101, DR Discounts 4102 when > 0, CR VAT Payable 2102; branch fallback invoice→order→first active) via manager-scoped `JournalService.createDraftIn`/`postIn` + stock-out (`sales_invoice` inventory txn, direction OUT, `base_quantity` incl. free units, `unit_cost` = item standard cost, from the required `inventoryLocationId`) + `invoiced_quantity` bump + `PlanLimitService.consumePeriodic('invoices_per_month')` — all atomic, `@PlanLimit` decorator not used
