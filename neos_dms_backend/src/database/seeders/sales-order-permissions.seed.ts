...
/**
 * Phase 6a: adds the `sales.order.*` codes, then re-ensures every base role's
 * mappings. Salesman gets order capture (create/read/update/confirm/cancel,
 * own-scoped) but NOT `complete`; manager gets everything via `sales.*` glob;
 * admin via `*`. Idempotent — mirrors the earlier backfills as a new version.
 */
export const salesOrderPermissionsSeed: Seed = {
  version: 18,
  name: 'sales-order-permissions-backfill',
  async run(manager) {
