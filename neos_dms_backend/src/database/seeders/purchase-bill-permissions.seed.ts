...
/**
 * Phase 7a: adds the `purchase.bill.post` code and re-ensures every base
 * role's mappings. Accountant and manager get full bill control via the
 * `purchase.bill.*` glob (create/read/update/post/void); warehouse manager
 * stays GRN-only; salesman has no purchase access. Idempotent.
 */
export const purchaseBillPermissionsSeed: Seed = {
  version: 21,
  name: 'purchase-bill-permissions-backfill',
  async run(manager) {
