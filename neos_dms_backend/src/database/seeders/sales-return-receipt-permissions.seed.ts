...
/**
 * Phase 6d: adds the `sales.return.*` (credit note) and `sales.receipt.*`
 * (customer receipt) codes and re-ensures every base role's mappings.
 * Accountant and manager get full return/receipt control via the globs;
 * warehouse manager stays inventory-only; salesman keeps invoice/order scope.
 * Idempotent.
 */
export const salesReturnReceiptPermissionsSeed: Seed = {
  version: 23,
  name: 'sales-return-receipt-permissions-backfill',
  async run(manager) {
