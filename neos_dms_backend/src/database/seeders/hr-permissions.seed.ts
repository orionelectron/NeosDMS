...
/**
 * Phase C1 backfill: adds the `hr` module + `hr.*` permission codes, the new
 * `manager` base role for every org, and re-ensures base-role mappings
 * (manager gets full hr; salesman gains self-service leave access). Idempotent —
 * mirrors v11 (field-permissions) as a new version so already-seeded DBs pick
 * up the diff.
 */
export const hrPermissionsSeed: Seed = {
  version: 12,
  name: 'hr-permissions-backfill',
  async run(manager) {
    await manager.query(
      `INSERT INTO modules (id, name, code, description)
       SELECT uuid_generate_v4(), 'HR', 'hr', 'Leave, travel and expense management'
