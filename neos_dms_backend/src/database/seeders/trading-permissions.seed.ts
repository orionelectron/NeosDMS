...
/**
 * Backfill for organizations created before Phase 4: inserts the new
 * `trading.item-category.*` / `trading.brand.*` / `trading.uom-conversion.*`
 * permission codes and re-ensures every org's base-role mappings against the
 * updated catalog (warehouse_manager now covers `trading.*`). Idempotent —
 * mirrors v5/v6 but as a new version so already-seeded DBs pick up the diff.
 */
export const tradingPermissionsSeed: Seed = {
  version: 10,
  name: 'trading-permissions-backfill',
  async run(manager) {
