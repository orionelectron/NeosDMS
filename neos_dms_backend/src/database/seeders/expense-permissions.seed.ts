...
/**
 * Phase 7: adds the `purchase.expense.*` codes and re-ensures every base
 * role's mappings. Accountant and manager get full expense control via the
 * `purchase.expense.*` glob (create/read/update/post/void); warehouse manager
 * and salesman stay out of expenses. Idempotent.
 */
export const expensePermissionsSeed: Seed = {
  version: 24,
  name: 'expense-permissions-backfill',
  async run(manager) {
