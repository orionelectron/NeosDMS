...
/**
 * Phase C2 backfill: adds the `hr.travel_request.*` and `hr.expense.*`
 * permission codes, then re-ensures every base role's mappings so salesman
 * gains self-service travel + expense access and accountant gains the
 * `hr.expense.pay` reimbursement step. Idempotent — mirrors v12 as a new
 * version so already-seeded DBs pick up the diff.
 */
export const travelPermissionsSeed: Seed = {
  version: 13,
  name: 'travel-permissions-backfill',
  async run(manager) {
