...
/**
 * Phase 5e: adds the `sales.target.*` permission codes, then re-ensures every
 * base role's mappings so salesman gains read-only access to their own targets
 * while manager/admin (via `sales.*`) can create, adjust and delete. Idempotent
 * — mirrors the earlier backfills as a new version.
 */
export const salesTargetPermissionsSeed: Seed = {
  version: 16,
  name: 'sales-target-permissions-backfill',
  async run(manager) {
