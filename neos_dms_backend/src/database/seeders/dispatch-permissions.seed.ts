...
/**
 * Phase 8a — dispatch permission backfill. Adds `dispatch.dispatch.*` and
 * `dispatch.vehicle.*` codes to the catalog, then re-ensures every base
 * role's mappings. Driver keeps read/update (own dispatch only — enforced in
 * the service); warehouse_manager and manager gain the full dispatch and
 * vehicle CRUD via `dispatch.dispatch.*` / `dispatch.vehicle.*`; admin via
 * `*`. Idempotent.
 */
export const dispatchPermissionsSeed: Seed = {
  version: 25,
  name: 'dispatch-permissions-backfill',
  async run(manager) {
